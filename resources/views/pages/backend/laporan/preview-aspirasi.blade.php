...
<div class="main-content">
    <section class="section">
        <div class="section-header bg-primary text-white p-3 rounded">
            <h1>Preview Laporan Aspirasi</h1>
        </div>

        <div class="section-body">
            <!-- Form Pencarian -->
            <div class="mb-4">
                <form method="GET" action="{{ route('laporan.preview-aspirasi') }}" class="d-flex gap-2 flex-wrap align-items-center">
                    <input type="text" name="search" class="form-control" placeholder="Cari laporan..." value="{{ $search ?? '' }}">
                    <input type="hidden" name="tahun" value="{{ $tahun }}">
                    <input type="hidden" name="bulan" value="{{ $bulan }}">
                    <button type="submit" class="btn btn-primary"><i class="fas fa-search"></i> Cari</button>
                    <a href="{{ route('laporan.preview-aspirasi', ['tahun' => $tahun, 'bulan' => $bulan]) }}" class="btn btn-secondary"><i class="fas fa-sync"></i> Reset</a>
                </form>
            </div>
            ...
            <div class="mb-3 d-flex flex-wrap gap-2">
                <form action="{{ route('laporan.export.pdf') }}" method="POST">
                    @csrf
                    <input type="hidden" name="tab" value="aspirasi">
                ...
                <a href="{{ route('laporan.index', ['tab' => 'aspirasi', 'search' => $search, 'tahun' => $tahun, 'bulan' => $bulan]) }}" class="btn btn-secondary btn-icon">
            ...
            <!-- Tabel Data -->
            @forelse($aspirasiByMonth as $month => $aspirasis)
                                ...
                                <tbody>
                                    @foreach($aspirasis as $index => $aspirasi)
                                        <tr>
                                            ...
                                            <td>
                                                <span class="badge {{ $aspirasi->status === 'selesai' ? 'badge-success' : 'badge-warning' }}">
                                                    {{ $aspirasi->status ?? 'Diproses' }}
                                                </span>
                                            </td>
                                            <td>{{ $aspirasi->created_at->format('Y-m-d H:i:s') }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
